...
public class GraficaManagedBean extends BaseManagedBean {

    @SuppressWarnings("compatibility:3779111823521269003")
    ...
    private List<String> crearUnaGraficaBar(GraficaDTO graficaDTO, DefaultCategoryDataset dataSetBar) {
        List<String> urlImagenes = new ArrayList<String>();
        listaIdGraficas = new ArrayList<BigDecimal>();
    ...
    private DefaultCategoryDataset generaDataSetNivel0Bar(GraficaDTO graficaDTO) {
        ...
        dataSet.addValue(graficaDTO.getListaValores().get(0).getCantidad().intValue(), graficaDTO.getTituloGrafica(), periodo.toString());
    ...
    private void valorMaximoEjeY(Integer cantidadBarra) {
        if (ejeYMaximo.intValue() < cantidadBarra) {
    ...
    private void valorMaximoFinalEjeY() {
        ...
        if (moduloEjeYMaximo.intValue() != 0 && moduloEjeYMaximo.intValue() != ConstantesReportes.N_5) {
        ...
        if (ejeYMaximo.intValue() % ConstantesReportes.N_6 != 0) {
            ...
            scalaEjeY = ejeYMaximo.intValue() / ConstantesReportes.N_6;
        } else {
            scalaEjeY = ejeYMaximo.intValue() / ConstantesReportes.N_6;
    ...
    private PieDataset generaDataSetNivel0Pie(List<GraficaValoresDTO> lista) {
        ...
        data.setValue("Insumos", lista.get(0).getCantidad().intValue());
        return data;
    }
        ...
        for (int i = 0; i < lista.size(); i++) {
            for (int j = i + 1; j < lista.size(); j++) {
                if (lista.get(i).getMetodo().equals(lista.get(j).getMetodo()) && (lista.get(i).getMes() == null || lista.get(i).getMes().equals(lista.get(j).getMes()))) {
                    ...
                    suma = suma + lista.get(j).getCantidad().intValue();
    ...
    private void sumarCantidadUnidad(List<GraficaValoresDTO> lista) {
        for (int i = 0; i < lista.size(); i++) {
            for (int j = i + 1; j < lista.size(); j++) {
                if (lista.get(i).getIdUnidad().equals(lista.get(j).getIdUnidad()) && (lista.get(i).getMes() == null || lista.get(i).getMes().equals(lista.get(j).getMes()))) {
                    Integer suma = lista.get(i).getCantidad();
                    suma = suma + lista.get(j).getCantidad().intValue();
                    lista.get(i).setCantidad(suma);
                    lista.remove(lista.get(j));
    ...
    private void guardarGraficaLista(JFreeChart jfreechart) {
        ...
        try {
            ...
            listaIdGraficas.add(getArchivoTempService().insertaArchivoTemp(archivoTemp));
        } catch (IOException e) {
    ...
    public StreamedContent getGrafica() {
        ...
        if (context.getCurrentPhaseId() == PhaseId.RENDER_RESPONSE) {
            ...
            return new DefaultStreamedContent(listaGraficas.get(contarGrafica++), "image/png");
        }
    }
