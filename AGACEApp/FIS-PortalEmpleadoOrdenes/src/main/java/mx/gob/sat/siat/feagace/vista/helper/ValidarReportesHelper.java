...
public class ValidarReportesHelper extends BaseHelper {
    ...
    @SuppressWarnings("compatibility:4167804588127011067")
    ...
    public ValidacionReportesDTO validarFormulario(ReportesVO vo) {
        ...
        if (vo != null && vo.getTipoReporteEjecutivoSeleccionado() != null && vo.getTipoReporteEjecutivoSeleccionado().trim().equals(FacesUtil.getMessageResourceString("lbl.reporte.titulo.gerencial"))) {
            logger.debug("Validacion Formulario Gerencial");
            this.getValidacion().setErrorTotal(this.validacionGerencial(vo));
        } else if (vo != null && vo.getTipoReporteEjecutivoSeleccionado() != null && vo.getTipoReporteEjecutivoSeleccionado().trim().equals(FacesUtil.getMessageResourceString("lbl.reporte.titulo.ejecutivo"))) {
    ...
    private int validacionEjecutivoRequeridos(ReportesVO vo) {
        ...
        if (vo.getTipoFecha() != null && !vo.getTipoFecha().trim().isEmpty()) {
            ...
            if ((vo.getTipoFecha().trim().equals(FacesUtil.getMessageResourceString(ConstantesReportes.TIPO_FECHA_FECHAS))) && (ValidacionParametrosUtil.seCumpleAlgunaCondicion(condicionesAlmenosUnaPrimarias))) {
                requerido++;
            } else if ((vo.getTipoFecha().trim().equals(FacesUtil.getMessageResourceString(ConstantesReportes.TIPO_FECHA_MESES))) && ValidacionParametrosUtil.seCumpleAlgunaCondicion(condicionesAlmenosUnaSecundarias)) {
                requerido++;
            }
        } else {
            requerido++;
    ...
    private int validacionFechasEjecutivo(ReportesVO vo) {
        ...
        if ((vo.getTipoFecha().trim().equals(FacesUtil.getMessageResourceString(ConstantesReportes.TIPO_FECHA_FECHAS))) && (vo.getFechaInicioPeriodo().after(vo.getFechaFinPeriodo()))) {

            requerido++;
            this.getValidacion().setMensaje("La fecha Per\u00edodo Fin no puede ser menor a la fecha Per\u00edodo Inicio.");

        }
        if (vo.getTipoFecha().trim().equals(FacesUtil.getMessageResourceString(ConstantesReportes.TIPO_FECHA_MESES))) {
