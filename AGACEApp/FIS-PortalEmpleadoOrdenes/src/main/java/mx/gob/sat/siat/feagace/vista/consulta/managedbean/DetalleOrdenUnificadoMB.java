...
import mx.gob.sat.siat.feagace.vista.consulta.DetalleOrdenAbstractMB;
...
public class DetalleOrdenUnificadoMB extends DetalleOrdenAbstractMB {
    ...
    public void init() {
        ...
        if (getSession().getAttribute(ORIGEN_CONSULTA) != null) {
            ...
            getHelper().setContribuyente(getConsultarReimprimirDocumentosService().buscarContribuyentePorID(getOrdenConsultaSeleccionada().getIdContribuyente()));
            ...
            for (FecetPromocion promocion : listFecetPromocion) {
                promocion.setDescripcionTipoPromocion(BusinessUtil.getTipoPromocionOrdenPorMetodo(getOrdenConsultaSeleccionada().getIdMetodo()));
                promocion.setExtemporanea(getPlazosService().esDocumentoExtemporaneoOrden(getHelper().getOrdenSeleccionada(), promocion.getFechaCarga()) ? "1" : "0");
            }
            List<PromocionConsultaDTO> listPromocionConsultaDTO = getConsultarReimprimirDocumentosHelper().convertToPromocionConsulta(listFecetPromocion, getOrdenConsultaSeleccionada());
            getOrdenConsultaSeleccionada().setListPromocionConsultaDTO(listPromocionConsultaDTO);
            // Obtener informacion de oficios.
            List<FecetOficio> listFecetOficio = getConsultarReimprimirDocumentosService().buscarOficiosEnviadosCont(idOrden);
            ...
            if (listFecetOficio != null) {
                ...
                listOficioConsultaDTO = getConsultarReimprimirDocumentosHelper().convertToOficioConsultaDTO(listFecetOficio, getOrdenConsultaSeleccionada());
            } else {
            ...
            if (getVistaHelper().isPnlOficiosFirm()) {
                getHelper().setListaOficiosFirmados(getSeguimientoOrdenesService()
                        .getOficiosFirmados(getOrdenConsultaSeleccionada().getIdOrden()));
            ...
            if (getVistaHelper().isPnlAsociados()) {
                obtenApoderadoLegalDelRL();
                obtenApoderadoLegal();
            }
        } else {
    ...
    public void redireccionaOficio() throws IOException {
        ...
        if (getHelper().getOficioConsultaSeleccionado() != null) {
            List<FecetOficio> listFecetOficio = getConsultarReimprimirDocumentosService().buscarOficioPorOrden(getOrdenConsultaSeleccionada().getIdOrden());
            for (FecetOficio oficio : listFecetOficio) {
        ...
        try {
            ...
            getSession().setAttribute("muestraPapelesTrabajo", (getEmpleadoService().validarExistenciaTipoEmpleado(empleado, TipoEmpleadoEnum.AUDITOR)));
    ...
    public void redireccionaCifras() throws IOException {
        ...
        String urlRedireccion = "/faces/firmante/cifras/cifras.xhtml?faces-redirect=true";
    ...
    public void cargaSolicitudContribuyente() {
        ...
        getSolHelper().setListaSolicitudContribuyenteConcentrado(getSolHelper().llenaObjetoSolicitudContFull(
                getSolHelper().getListaSolicitudContribuyenteConcentrado(),
        ...
        if (getSolHelper().getListaProrrogaHistorico() != null && !getSolHelper().getListaProrrogaHistorico().isEmpty()) {
            for (FecetProrrogaOrden prorroga : getSolHelper().getListaProrrogaHistorico()) {
                if (prorroga.getFececEstatus().getIdEstatus().equals(EstatusProrroga.RESOLUCION_PRORROGA_FIRMADA_ENVIADA_NYV.getBigIdEstatus()) || prorroga.getFececEstatus().getIdEstatus().equals(EstatusProrroga.PRORROGA_NOTIFICADA_CONTRIBUYENTE.getBigIdEstatus())) {
                    if (prorroga.getAprobada()) {
                        prorroga.setDescripcionEstado(FacesUtil.getMessageResourceString("estatus.firmante.aprobada"));

                    } else {
                        prorroga.setDescripcionEstado(FacesUtil.getMessageResourceString("estatus.firmante.no.aprobada"));
                    }
                }
            }
        }
        ...
        getSolHelper().setListaPruebasPericiales(
                getSeguimientoOrdenesService().getPruebaPericialPorOrdenEstatusPendienteAuditor(
                        getHelper().getOrdenSeleccionada().getIdOrden()));
        ...
        getSolHelper().setListaPruebasPericialesHistorico(
                getSeguimientoOrdenesService().getHistoricoPruebasPericiales(
                        getHelper().getOrdenSeleccionada().getIdOrden()));

        getSolHelper().setListaSolicitudContribuyenteHistorico(
                getSolHelper().llenaObjetoSolicitudContPruebasPeri(getSolHelper().getListaPruebasPericialesHistorico()));
    ...
    public void obtenRepresentanteLegal() {
        ...
        getHelper().getColaboradoresDTO().setRepresentanteLegal(
                (getAsociadosService().cargaColaborador(repLegal, convertirDTOaAgaceOrden(getOrdenConsultaSeleccionada()))));
    ...
    public void obtenApoderadoLegal() {
        ...
        getHelper().getColaboradoresDTO().setApoderadoLegal(
                (getAsociadosService().obtenerApoderadoLegalContribuyente(getHelper().getOrdenSeleccionada().getFecetContribuyente().getRfc(), apoderadoLegal)));
        cargarDocApoderadoLegal();
    }

    public void obtenApoderadoLegalDelRL() {
        ColaboradorVO apoderadoLegalDelRL = new ColaboradorVO();
        apoderadoLegalDelRL.setTipoAsociado(Constantes.ID_APODERADO_LEGAL_REPRESENTANTE_LEGAL);
        getHelper().getColaboradoresDTO().setApoderadoLegalRepresentanteLegal(
                (getAsociadosService().cargaColaborador(apoderadoLegalDelRL, getHelper().getOrdenSeleccionada())));
        cargarDocRepresentanteLegalAL();
    }
    ...
    private void cargarDocRepresentanteLegal() {
        if (getHelper().getColaboradoresDTO().getRepresentanteLegal() != null
                && getHelper().getColaboradoresDTO().getRepresentanteLegal().getAsociado() != null) {
            getHelper().getColaboradoresDTO().setDocumentosRepresentante(
    ...
    private void cargarDocRepresentanteLegalAL() {
        if (getHelper().getColaboradoresDTO().getApoderadoLegalRepresentanteLegal() != null
                && getHelper().getColaboradoresDTO().getApoderadoLegalRepresentanteLegal().getAsociado() != null) {
            getHelper().getColaboradoresDTO().setDocumentosALRL(
                    getFecetDocAsociadoService().obtenerDocumentosPorAsociado(
                            getHelper().getColaboradoresDTO().getApoderadoLegalRepresentanteLegal().getAsociado().getIdAsociado()));
        } else {
            getHelper().getColaboradoresDTO().setDocumentosALRL(new ArrayList<FecetDocAsociado>());
        }
    }

    private void cargarDocApoderadoLegal() {
        if (getHelper().getColaboradoresDTO().getApoderadoLegal() != null
                && getHelper().getColaboradoresDTO().getApoderadoLegal().getAsociado() != null) {
            getHelper().getColaboradoresDTO().setDocumentosAL(
                    getFecetDocAsociadoService().obtenerDocumentosPorAsociado(
                            getHelper().getColaboradoresDTO().getApoderadoLegal().getAsociado().getIdAsociado()));
        } else {
            getHelper().getColaboradoresDTO().setDocumentosAL(new ArrayList<FecetDocAsociado>());
        }
    }
    ...
    public void cargarPapelesTrabajoOrden() {
        ...
        List<PapelesTrabajo> listaPapelTrabajo = getConsultarPapelesTrabajoService().getPapelesByIdPropuestaOrIdOrden(
                getOrdenConsultaSeleccionada().getIdPropuesta(),
                getOrdenConsultaSeleccionada().getIdOrden());
    ...
    public StreamedContent getArchivoDescargaPapelTrabajo() {
        ...
        if (getHelper().getPapelTrabajoSeleccionado().getPapelesTrabajo().getRutaArchivo() == null) {
            ...
            return new DefaultStreamedContent(myInputStream, "application/octet-stream", getHelper().getPapelTrabajoSeleccionado().getPapelesTrabajo().getNombreArchivo());
        }
    ...
    public void buscarDocPruebasAlegatos() {
        List<FecetAlegato> listFecetAlegato = getConsultarReimprimirDocumentosService().buscarAlegatosPorPromocion(getHelper().getPromocionConsultaSeleccionada().getIdPromocion());
    ...
    public void buscarProrrogaOficio() {
        List<FecetProrrogaOficio> listFecetProrrogaOficio = getConsultarReimprimirDocumentosService().buscarProrrogaOficioPorOficio(getHelper().getOficioConsultaSeleccionado().getIdOficio());
        List<ProrrogaOficioConsultaDTO> listProrrogaOficioConsulta = getConsultarReimprimirDocumentosHelper().convertToProrrogaOficioConsultaDTO(listFecetProrrogaOficio,
        ...
        for (ProrrogaOficioConsultaDTO prorrogaOficioConsulta : listProrrogaOficioConsulta) {
            ...
            List<FecetFlujoProrrogaOficio> listFecetFlujoProrrogaOficios = getConsultarReimprimirDocumentosService().buscarFlujoProrrogaOficioPorProrroga(idOficioProrroga);
    ...
    public void buscarAnexosOficios() {
        List<FecetOficioAnexos> listFecetOficioAnexo = getConsultarReimprimirDocumentosService().buscarOficioAnexoPorOficio(getHelper().getOficioConsultaSeleccionado().getIdOficio());
    ...
    public void buscarOficiosDependientes() {
        List<FecetOficio> listOficioDependiente = getConsultarReimprimirDocumentosService().buscarOficioDependientePorOficio(getHelper().getOficioConsultaSeleccionado().getIdOficio());
        getHelper().getOficioConsultaSeleccionado().setListOficioDependientes(getConsultarReimprimirDocumentosHelper().convertToOficioConsultaDTO(listOficioDependiente, getOrdenConsultaSeleccionada()));
    }
    ...
    public void buscarDocPruebasAlegatosOficio() {
        List<FecetAlegatoOficio> listFecetAlegatoOficio = getConsultarReimprimirDocumentosService()
                .buscarAlegatoOficioPorPromocionOficio(getHelper().getPromocionOficioConsultaSeleccionada().getIdPromocionOficio());
    ...
    public void getAnexosProrrogaOrden() {
        getSolHelper().setListaAnexosSolicitudContribuyenteHistorico(getSolHelper().llenaAnexoSolicitudContribuyenteProrroga(getSeguimientoOrdenesService().getAnexosProrrogaOrdenHistorico(
                getSolHelper().getSolicitudContribuyenteVOSeleccionada().getProrrogaOrden().getFecetFlujoProrrogaOrden().getIdFlujoProrrogaOrden())));
    }

    public void getAnexosPruebasPericiales() {
        getSolHelper().setListaAnexosSolicitudContribuyenteHistorico(getSolHelper().llenaAnexoSolicitudContribuyentePruebasPericiales(getSeguimientoOrdenesService().getAnexosPruebasPericialesHistorico(
                getSolHelper().getSolicitudContribuyenteVOSeleccionada().getPruebasPericiales().getFlujoPruebasPericiales().getIdFlujoPruebasPericiales())));
    }
    ...
    public void buscarDocProrrogaOficio() {
        List<FecetDocProrrogaOficio> listFecetDocProrrogaOficio = getConsultarReimprimirDocumentosService()
                .buscarDocProrrogaOficioPorProrroga(getHelper().getProrrogaOficioConsultaSeleccionada().getIdProrroga());
    ...
    public void buscarDocAnexoResolProrrogaOficio() {
        BigDecimal idFlujoProrrogaOficio = getHelper().getFlujoProrrogaOficioConsultaDTOSeleccionada().getIdFlujoProrrogaOficio();
        List<FecetAnexosProrrogaOficio> listFecetAnexosProrrogasOficios = getConsultarReimprimirDocumentosService().buscarAnexosProrrogaOficioPorFlujoProrrogaOficio(idFlujoProrrogaOficio);
        List<AnexoProrrogaOficioConsultaDTO> listAnexosProrrogasOficiosConsulta = getConsultarReimprimirDocumentosHelper().convertToAnexoProrrogaOficioConsultaDTO(listFecetAnexosProrrogasOficios);
        String nombreAuditor = "";
        BigDecimal idAuditor = getHelper().getProrrogaOficioConsultaSeleccionada().getIdAuditor() != null ? getHelper().getProrrogaOficioConsultaSeleccionada().getIdAuditor() : getOrdenConsultaSeleccionada().getIdAuditor();
        if (idAuditor != null) {
        ...
        getHelper().getFlujoProrrogaOficioConsultaDTOSeleccionada().setListAnexosProrrogaOficios(listAnexosProrrogasOficiosConsulta);
    }
    ...
    public void getDocumentacionProrrogaContribuyente() {
        ...
        getSolHelper().setListaSolicitudContribuyenteDocVO(getSolHelper().llenaListaSolicitudContribuyenteDocVO(getSolHelper().getListaDocumentosProrroga(), null));
    }

    public void getDocumentacionPruebaPericialContribuyente() {
        getSolHelper().setListaDocumentosPruebaPericial(getSeguimientoOrdenesService().getDocumentacionPruebaPericialContribuyente(
                getSolHelper().getSolicitudContribuyenteVOSeleccionada().getPruebasPericiales().getIdPruebasPericiales()));
        getSolHelper().setListaSolicitudContribuyenteDocVO(getSolHelper().llenaListaSolicitudContribuyenteDocVO(null, getSolHelper().getListaDocumentosPruebaPericial()));
    }
    ...
    public StreamedContent getArchivoDescargaResolucionSolicitudContribuyente() {
        if (getSolHelper().getSolicitudContribuyenteVOSeleccionada().getProrrogaOrden() != null) {
            ...
            return getArchivoGenerico(getSolHelper().getSolicitudContribuyenteVOSeleccionada().getPruebasPericiales().getRutaResolucion(),
                    getSolHelper().getSolicitudContribuyenteVOSeleccionada().getPruebasPericiales().getNombreResolucion());
        }

    }

    public StreamedContent getArchivoDescargaResolucionProrrogaOrden() {
        return getArchivoGenerico(getSolHelper().getSolicitudContribuyenteVOSeleccionada().getProrrogaOrden().getRutaResolucion(),
    ...
    public StreamedContent getArchivoDescargaAnexoSolicitudContribuyente() {
        if (getSolHelper().getSolicitudContribuyenteVOSeleccionada().getProrrogaOrden() != null) {
            ...
            return getArchivoGenericoAnexoProrrogaOrden(getSolHelper().getSolicitudContribuyenteAnexoHistoricoVOSeleccionada().getFecetAnexoPruebasPericiales().getRutaArchivo(),
                    getSolHelper().getSolicitudContribuyenteAnexoHistoricoVOSeleccionada().getFecetAnexoPruebasPericiales().getNombreArchivo());
        }

    }

    public StreamedContent getArchivoDescargaAnexoProrrogaOrden() {
        return getArchivoGenericoAnexoProrrogaOrden(getSolHelper().getSolicitudContribuyenteAnexoHistoricoVOSeleccionada().getFecetAnexosProrrogaOrden().getRutaArchivo(),
                getSolHelper().getSolicitudContribuyenteAnexoHistoricoVOSeleccionada().getFecetAnexosProrrogaOrden().getNombreArchivo());
    }
    ...
    public StreamedContent getArchivoAnexoResolucionProrrogaOrden() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getOrdenConsultaSeleccionada().getIdOrden());
        getPistaAuditoriaDescargaDocumentosService().consultarAnexoResolucionProrroga(orden);
        content = getArchivoDescargaAnexoSolicitudContribuyente();
        return content;
    }

    public StreamedContent getArchivoRepresentanteLegal() {
        StreamedContent content = null;
        try {
            AgaceOrden orden = new AgaceOrden();
            orden.setIdOrden(getOrdenConsultaSeleccionada().getIdOrden());
            getPistaAuditoriaDescargaDocumentosService().consultarRepresentanteLegal(orden);
            getArchivoGenerico(getRutaArchivo(), getNombreArchivo());
        } catch (final Exception exception) {
            logger.error("No se pudo descargar el archivo. ", exception);
            addErrorMessage("No se encontro el documento seleccionado");
        }
    ...
    public StreamedContent getArchivoPapelesTrabajoOrden() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
        getPistaAuditoriaDescargaDocumentosService().consultarPapelTrabajo(orden);
        content = getArchivoDescargaPapelTrabajo();
        return content;
    }
