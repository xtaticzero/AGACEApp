...
import mx.gob.sat.siat.feagace.vista.consulta.helper.DetalleOficioUnifHelper;
...
public class DetalleOficioUnificadoMB extends ReimprimirDocumentosAbstractMB {
    ...
    protected static final String REIMPRIMIR_DOCS = "consultarReimprimirDocs";
    ...
    public StreamedContent getArchivoPromocionOficio() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
        String origenConsulta = (String) getSession().getAttribute(ORIGEN_CONSULTA);
        if (origenConsulta.equalsIgnoreCase(REIMPRIMIR_DOCS)) {
            getPistaAuditoriaDescargaDocumentosService().consultarPromocionOficioInterno(orden);
        } else {
            getPistaAuditoriaDescargaDocumentosService().consultarPromocionOficio(orden);
        }
        content = getArchivoDescargaPromocionOficio();
        return content;
    }

    public StreamedContent getArchivoAnexoPromocionOficio() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
        getPistaAuditoriaDescargaDocumentosService().consultarPromocionAnexoOficio(orden);
        content = getArchivoDescargaPruebasAlegatos();
        return content;
    }

    public StreamedContent getArchivoProrrogaOficio() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
        String origenConsulta = (String) getSession().getAttribute(ORIGEN_CONSULTA);
        if(origenConsulta.equalsIgnoreCase(REIMPRIMIR_DOCS)){
            ...
            getPistaAuditoriaDescargaDocumentosService().consultarPromocionOficioProrroga(orden);
        }
        content = getArchivoDescargaDocumentacionProrroga();
        return content;
    }

    public StreamedContent getArchivoResolucionProrrogaOficio() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
        String origenConsulta = (String) getSession().getAttribute(ORIGEN_CONSULTA);
        if (origenConsulta.equalsIgnoreCase(REIMPRIMIR_DOCS)) {
            ...
            getPistaAuditoriaDescargaDocumentosService().consultarOficioProrrogaResolucion(orden);
        }
        content = getArchivoDescargaResolucionProrrogaOficio();
        return content;
    }

    public StreamedContent getArchivoAnexoResolucionProrrogaOficio() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
        getPistaAuditoriaDescargaDocumentosService().consultarOficioAnexoProrrogaResol(orden);
        content = getArchivoAnexoProrrogaOficio();
        return content;
    }

    public StreamedContent getArchivoPapelesTrabajoOficio() {
        StreamedContent content = null;
        AgaceOrden orden = new AgaceOrden();
        orden.setIdOrden(getHelper().getOrdenSeleccionada().getIdOrden());
