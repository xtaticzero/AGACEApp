...
@ManagedBean(name = "firmaPromocionCompulsaManagedBean")
@ViewScoped
public class FirmaPromocionCompulsaManagedBean extends FirmaManagedBean {

    @SuppressWarnings("compatibility:-8122401672310613532")
    ...
    public void iniciarFirmas() {
        setDocs((PromocionDocsVO) getSession().getAttribute("promocionCompulsaFirmar"));
        getAuditoriaService().cargarPromocionOficio(((PromocionDocsVO)getDatosFirma()).getOficioSeleccionado());  
        super.iniciarFirmas();
    }

    @Override
    public String regresar() {
        getSession().removeAttribute("promocionCompulsaFirmar");
        File acuse = getAcuse();
        if (acuse != null) {
            try {
            ...
            getSession().setAttribute("promocionCompulsa", getAcuseFilePromocion());
        }
