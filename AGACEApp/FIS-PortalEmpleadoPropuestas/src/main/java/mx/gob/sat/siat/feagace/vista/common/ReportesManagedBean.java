...
public class ReportesManagedBean extends ReportesManagedBeanAbstract {
    ...
    private boolean validaTipoBusqueda() {
        if (getTipoBusqueda() == null) {
            addErrorMessage(null, "Debe seleccionar un tipo de b�squeda.");
            return false;
        }
        return true;
    }
    ...
    private boolean validaMetodoOrden() {
        ...
        if (getMetodoSeleccionado().intValue() == -1) {
            addErrorMessage(null, "Debe seleccionar un método para realizar la consulta");
            valida = false;
        } else if (getCveOrden() == null || getCveOrden().trim().length() < Constantes.ENTERO_TRECE) {
            addErrorMessage(null, "N�mero de �rden incorrecta");
            valida = false;
        }
        return valida;
    }
    ...
    private boolean validaMetodoOrdenEstado() {
        ...
        if (getMetodoSeleccionado().intValue() == -1) {
            ...
            addErrorMessage(null, "Estatus incorrecta");
            valida = false;
        }
        return valida;
    }
    ...
    public boolean validaRango() {
        ...
        error.append(getMessageResourceString("error.campo.obligatorio")).append(": ");
        ...
        if (!valida) {
            addErrorMessage(null, error.toString());
        }
