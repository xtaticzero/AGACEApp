...
import mx.gob.sat.siat.feagace.vista.propuestas.consulta.ReimprimirDocumentosAbstractMB;
import mx.gob.sat.siat.feagace.vista.propuestas.consulta.helper.DetalleOficioUnifHelper;
...
public class DetalleOficioUnificadoMB extends ReimprimirDocumentosAbstractMB {
    ...
    public void init() {
        ...
        if (getSession().getAttribute("oficioSeleccionadoU") != null) {
            ...
            List<FecetPromocionOficio> listaPromociones = getSeguimientoOrdenesService()
                    .getPromocionContadorPruebasAlegatosOficio(getOficioSeleccionado());
            for (FecetPromocionOficio promocion : listaPromociones) {
                promocion.setDescripcionTipoPromocion(BusinessUtil.getTipoPromocionPorTipoOficioMetodo(
                        getOficioSeleccionado().getFecetTipoOficio().getIdTipoOficio()));
                if (getOficioSeleccionado().getFecetTipoOficio().getIdTipoOficio()
                        .equals(TiposOficiosOrdenesEnum.OTRAS_AUTORIDADES.getBigIdTipoOficio())
                        || getOficioSeleccionado().getFecetTipoOficio().getIdTipoOficio()
                    ...
                    if (getValidarOficioAdministrable().esAdministrable(getOficioSeleccionado())) {
                        if (getValidarOficioAdministrable().tienePlazos(getOficioSeleccionado())) {
                            promocion.setExtemporanea(getPlazosService().esDocumentoExtemporaneoOficio(
                                    getOficioSeleccionado(), promocion.getFechaCarga()) ? "1" : "0");
                        } else {
                            promocion.setExtemporanea("0");
                        }
                    } else {
            ...
            getHelper().setListaProrroga(getSeguimientoOrdenesService()
                    .getProrrogaPorOficioEstatusPendienteAuditor(getOficioSeleccionado().getIdOficio()));

            getHelper().setListaProrrogaHistorico(
                    getSeguimientoOrdenesService().getHistoricoProrrogaOficio(getOficioSeleccionado().getIdOficio()));
    ...
    public void cargarPapelesTrabajoOficio() {
        ...
        List<PapelesTrabajo> listaPapelTrabajo = getConsultarPapelesTrabajoService()
                .getPapelesOnlyIdOfcio(getOficioSeleccionado().getId());
    ...
    public void getDocumentacionRechazoProrroga() {
        getHelper().setListaDocumentosRechazoProrroga(getSeguimientoOrdenesService().getAnexosRechazoProrrogaOficio(
                getHelper().getProrrogaOficioSeleccionada().getFecetFlujoProrrogaOficio().getIdFlujoProrrogaOficio()));
    }

    public void getDocumentacionProrrogaContribuyente() {
        getHelper().setListaDocumentosProrroga(getSeguimientoOrdenesService().getDocumentacionProrrogaContribuyente(
                getHelper().getProrrogaOficioSeleccionada().getIdProrrogaOficio()));
        // getDtoHelper().getSolicitudContribuyenteVOSeleccionada().getProrrogaOrden().getIdProrrogaOrden()))
        // LINEA ORIGINAL
        getHelper().setListaSolicitudContribuyenteDocVO(
                getHelper().llenaListaSolicitudContribuyenteDocVO(getHelper().getListaDocumentosProrroga(), null));
    }

    public void getPruebasAlegatosPromocion() {
        getHelper().setListaPruebasAlegatos(getCargarFirmaPruebasPromoService()
                .getPruebasAlegatosPromocionOficio(getHelper().getPromocionSeleccionada().getIdPromocionOficio()));
    ...
    public void getAnexosProrrogaOficio() {
        getHelper().setListaAnexosProrrogaOficio(getSeguimientoOrdenesService().getAnexosProrrogaOficioHistorico(
                getHelper().getProrrogaOficioSeleccionada().getFecetFlujoProrrogaOficio().getIdFlujoProrrogaOficio()));
    }
    ...
    public StreamedContent getArchivoAnexoProrrogaOficio() {
        this.archivoAnexoProrrogaOficio = getDescargaArchivo(
                getHelper().getAnexoProrrogaOficioSeleccionado().getRutaArchivo(),
    ...
    public StreamedContent getArchivoDescargaDocumentacionProrroga() {
        this.archivoDescargaDocumentacionProrroga = getDescargaArchivo(
                getHelper().getDocumentacionProrrogaOficioSeleccionado().getRutaArchivo(),
    ...
    public StreamedContent getArchivoDescargaAnexoProrrogaOficio() {
        archivoDescargaAnexoProrrogaOficio = getDescargaArchivo(
                getHelper().getAnexoProrrogaOficioSeleccionado().getRutaArchivo(),
                getHelper().getAnexoProrrogaOficioSeleccionado().getNombreArchivo());
    ...
    public StreamedContent getArchivoDescargaResolucionProrrogaOficio() {
        this.archivoDescargaResolucionProrrogaOficio = getDescargaArchivo(
                getHelper().getProrrogaOficioSeleccionada().getRutaResolucion(),
    ...
    public StreamedContent getArchivoDescargaAcuseProrrogaOficio() {
        this.archivoDescargaAcuseProrrogaOficio = getDescargaArchivo(
                getHelper().getProrrogaOficioSeleccionada().getRutaAcuse(),
    ...
    public StreamedContent getArchivoDescargaPromocionOficio() {
        this.archivoDescargaPromocionOficio = getDescargaArchivo(
                getHelper().getPromocionSeleccionada().getRutaArchivo(),
