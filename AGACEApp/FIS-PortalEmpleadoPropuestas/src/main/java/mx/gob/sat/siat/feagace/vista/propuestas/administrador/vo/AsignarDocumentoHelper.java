...
public abstract class AsignarDocumentoHelper {
    ...
    public static List<PropuestaVO> llenarListaDeDto(List<FecetPropuesta> lista) {
        ...
        for (FecetPropuesta fecProp : lista) {
            ...
            prop.setPrioridad(fecProp.getPrioridadSugerida());
            prop.setSubprograma(fecProp.getFececSubprograma() != null ? fecProp.getFececSubprograma().getClave()
                    .concat("-").concat(fecProp.getFececSubprograma().getDescripcion()) : "");
            prop.setMetodo(fecProp.getFeceaMetodo() != null ? fecProp.getFeceaMetodo().getAbreviatura() : "");
            prop.setRfcContribuyente(
                    fecProp.getFecetContribuyente() != null ? fecProp.getFecetContribuyente().getRfc() : "");
            prop.setIdContribuyente(fecProp.getIdContribuyente());
            prop.setRfcAuditor(fecProp.getRfcAuditor());
            prop.setRfcFirmante(fecProp.getRfcFirmante());
            prop.setRfcSubadmin(fecProp.getRfcSubadministrador());
            prop.setRfcCreacion(fecProp.getRfcCreacion());
            prop.setRfcAdminEmision(fecProp.getRfcAdministrador());
            prop.setIdArace(fecProp.getIdArace());
    ...
    public static List<SubAdministradorVO> obtenerSubAministradores(List<EmpleadoDTO> listaSubs) {

        List<SubAdministradorVO> listaSubAdmin = new ArrayList<SubAdministradorVO>();
        for (EmpleadoDTO empleado : listaSubs) {
    ...
    public static String crearMensajeAprobacion1(String idRegistro) {
        String mensaje = FacesUtil.getMessageResourceString("mensaje.dialog.aprobar.msg1", idRegistro);
    ...
    public static Boolean validaArchivoCargaInsumoPropuesta(final UploadedFile archivo) {
        if (archivo.getFileName().endsWith(Constantes.ARCHIVO_WORD_DESPUES_2007)
            ...
            FacesUtil.addErrorMessage(null, "Archivo invalido",
